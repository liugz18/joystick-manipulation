...
def parse_radio_data(data_string):
    ...
    return result
def imageprocess(bg,image):
    background=cv2.cvtColor(bg,cv2.COLOR_RGB2GRAY)
    raw=cv2.cvtColor(image,cv2.COLOR_RGB2GRAY)
    ...
    circles= cv2.HoughCircles(img_bin,cv2.HOUGH_GRADIENT,1,30,param1=10,param2=8,minRadius=5,maxRadius=20)
    result=raw
...
#    swift.set_position(z=75,speed=1000)
    ...
    count = 0
    mode=1
    ret,background=sensor_cap.read()
    while True:
...
#        print(pos)
        ...
        print(count)
        if ret:           
            result=imageprocess(background,frame)
            result=cv2.cvtColor(result, cv2.COLOR_GRAY2BGR)
            sensor_writer.write(result)
            cv2.imshow('sensor data', result)
